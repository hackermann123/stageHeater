...
- JSON file format: {"temperature_c": 25.5, "heater_state": "On", "timestamp": 1234567890.123}
"""
...
JSON_OUTPUT_FILE = "/tmp/heater_thermistor.json" # File to write heater temp + state
...
def write_heater_json(temperature_c, heater_state):
	...
	try:
		...
		if isinstance(heater_state, bool):
			state_str = "On" if heater_state else "Off"
		else:
			state_str = str(heater_state)
...
def main():
	...
	print(f"JSON Format: {{\"temperature_c\": float, \"heater_state\": \"On\"|\"Off\", \"timestamp\": float}}")
	print("="*80 + "\n")
	...
	try:
		...
		while True:
			...
			if current_temp > MAX_TEMPERATURE:
				...
				set_relay(False)
				write_heater_json(current_temp, False)  # MODIFIED: Include state
				break
		...
		print("\n\n✓ Stopped by user")
		set_relay(False)
		write_heater_json(0, False)  # MODIFIED: Include state
	except Exception as e:
		print(f"\n✗ FATAL ERROR: {e}")
		import traceback
		traceback.print_exc()
